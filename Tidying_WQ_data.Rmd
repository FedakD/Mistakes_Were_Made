...
for(i in 1:length(parameters)){
  ...
  #Derek -- parameter -> parameters changed
  conc.list[[i]] <- readWQPdata(site.args) %>%
    mutate(parameters=names(parameters)[i]) #Mutate just adds a new column to the data frame
...
```{r site info}
#Derek site..info -> site.info changed
#head(site.info)
```
...
conc.tidy <- conc.clean %>% 
  ...
  select(date,
         ...
         SiteID,
         conc=value)
#Derek units = 'tons/day' -> units != 'tons/day' changed
